...
test('the product page states what it is and links to the download', async ({ page }) => {
  ...
  // Exact name, not /download/i -- the side nav added by the plugin-page quick
  // nav also renders a "Download" link, so a loose match now resolves to two.
  await expect(page.getByRole('link', { name: 'Download Seriatim' })).toBeVisible();
